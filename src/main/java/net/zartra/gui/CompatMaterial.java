...
final class CompatMaterial {
 ...
 static Material findOrNull(String name){if(name==null||name.trim().length()==0)return null;try{Material material=Material.matchMaterial(name.trim());if(material!=null)return material;}catch(Throwable ignored){}try{return Material.valueOf(name.trim().toUpperCase(java.util.Locale.ENGLISH));}catch(Throwable ignored){}if("WATCH".equalsIgnoreCase(name))return byName("CLOCK");if("GRASS".equalsIgnoreCase(name))return byName("SHORT_GRASS");if("INK_SACK".equalsIgnoreCase(name))return byName("BLACK_DYE");if("REDSTONE_COMPARATOR".equalsIgnoreCase(name))return byName("COMPARATOR");if("PLAYER_HEAD".equalsIgnoreCase(name))return byName("SKULL_ITEM");if("SKULL_ITEM".equalsIgnoreCase(name))return byName("PLAYER_HEAD");if("RED_STAINED_GLASS_PANE".equalsIgnoreCase(name))return byName("STAINED_GLASS_PANE");if("STAINED_GLASS_PANE".equalsIgnoreCase(name))return byName("RED_STAINED_GLASS_PANE");return null;}
